...
function eDomoticzBasePlatform(log, config, api) {
    ...
    if (this.api) {
        this.api.once("didFinishLaunching", function() {
            ...
            if (config.mqtt) {
                this.mqtt = eDomoticzBasePlatform.setupMqttConnection(this);
                this.mqtt.on('message', function(message) {
                    var accessory = this.accessories.find(function(acc) {
                        return (acc.idx == message.idx || acc.idx2 == message.idx);
                    });
...
eDomoticzBasePlatform.setupMqttConnection = function(platform) { // Setup the static MQTT client (shared) instance.
    if(eDomoticzBasePlatform.mqtt) {
        return eDomoticzBasePlatform.mqtt;
    }
    ...
    eDomoticzBasePlatform.mqtt = new Mqtt(platform.log);
    ...
    Domoticz.hardware(platform.apiBaseURL, function(hardware) {
        ...
        eDomoticzBasePlatform.mqtt.connect(connectionInformation.host, connectionInformation.port, connectionInformation.topic, {
