...
function eDomoticzTVAccessory(platform, platformAccessory, idx, name, uuid, swTypeVal, hwId, hwType, image) {
    ...
    if (!this.platformAccessory) {
        if (this.hwId.toString() in tvAccessories) {
            platform.log("platformAccessory: TV USE tvAccessories"); // TODO: Remove this line
            this.platformAccessory = tvAccessories[this.hwId.toString()].platformAccessory;
        } else if (this.hwId.toString() in tvInputAccessories) {
            platform.log("platformAccessory: TV USE tvInputAccessories"); // TODO: Remove this line
            this.platformAccessory = tvInputAccessories[this.hwId.toString()].platformAccessory;
        } else {
            platform.log("platformAccessory: TV CREATE NEW"); // TODO: Remove this line
            ...
            this.platformAccessory = new platform.api.platformAccessory(
                this.name,
                UUID.generate('domoticz:tv:' + this.name),
...
eDomoticzAccessory.prototype = {
    ...
    handleMQTTMessage: function (message, callback) {
        ...
        switch (true) {
        case this.isTelevisionDevice(this.swTypeVal):
            {
                // TV power state.
        ...
        case this.isTelevisionSourceDevice(this.swTypeVal, this.image):
            {
    ...
    getServices: function () {
        ...
        informationService.setCharacteristic(Characteristic.Manufacturer, "eDomoticz TV").setCharacteristic(Characteristic.Model, this.Type).setCharacteristic(Characteristic.SerialNumber, "Domoticz IDX " + this.idx);
...
eDomoticzTVAccessory = {
    isTelevisionDevice: function(swTypeVal) {
        return (swTypeVal == Constants.DeviceTypeMedia);
    },
    isTelevisionSourceDevice: function(swTypeVal, image) {
        return (swTypeVal == Constants.DeviceTypeSelector && image == "TV");
    },
    isTelevisionAccessory: function(swTypeVal, image) {
